    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                (text_width, text_height), _ = cv2.getTextSize(timestamp, cv2.FONT_HERSHEY_SIMPLEX, 1.5, 5)
                x = frame.shape[1] - text_width - 20  # 20 px from right
                y = frame.shape[0] - 20               # 20 px from bottom

                cv2.putText(frame, timestamp, (x, y), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 0), 5, cv2.LINE_AA)

              
                x = frame.shape[1] - text_width - 200  # 20 px from right
                y = frame.shape[0] - 20               # 20 px from bottom

                cv2.putText(frame, str(frame_num), (x, y), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 0), 5, cv2.LINE_AA)
