...
def toggle_recording(self):
    ...
    if self.cap is not None:
        if self.recorder and self.recorder.running:
            ...
            self.tbprint("Video Recording Stopped")
            self.tbprint("Video Saved to: {}".format(self.file_path))
        else:
            # Start recording
            ...
            self.file_path = os.path.join(self.new_dir_path, self.output_file_name + ".mp4")
            self.recorder = RecordThread(self.frame_queue, self.file_path, self.videofps)
            self.recorder.start()
