...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        for i in range(6):
            curve = self.plot_widget.plot(self.data_buffers[i], pen=pg.mkPen(colors[i], width=3),
                                          name=labels[i])
...
############################################################################################################################################################
        ...
        if self.ui.toggledisplayvisualscheckbox.isChecked():
            if self.control_status == True or self.joystick_status == True or self.manual_status == True or self.excel_actions_status == True :
                ...
                cv2.putText(displayframe, uniformfield,
                    (int(self.video_width / 1.8),int(self.video_height / 13)),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    fontScale=1.5, 
                    thickness=5,
                    color = (0, 0, 0),
    ...
    def start_data_record(self):
        ...
        - Cell tracking data
        """
        self.tbprint("Recording Excel Data...")
        self.frame_number = 0
        ...
        self.magnetic_field_actions_sheet = self.output_workbook.create_sheet(title="Magnetic Field Actions")#self.output_workbook.active
        self.magnetic_field_actions_sheet.append(["Frame", "Time(s)", "Bx (%)", "By (%)", "Bz (%)", "Alpha (rad)", "Gamma (rad)", "Rolling Frequency (Hz)", "Psi (rad)", "Gradient?","Equal Field?", "Acoustic Frequency (Hz)"])


        #create sheet for magneti field actions
        self.sensor_sheet = self.output_workbook.create_sheet(title="Sensors")#self.output_workbook.active
        self.sensor_sheet.append(["Time (s)", "I(A) | +Y Coil", "I(A) | +X Coil", "I(A) | -Y Coil", "I(A) | -X Coil", "I(A) | +Z Coil", "I(A) | -Z Coil"])
...
"                padding: 6px;")
    ...
    def populate_serial_ports(self):
        ...
        if len(port_names) >= 1:
            if port_names[0].startswith("COM") or port_names[0].startswith("/dev/us.usb"):
                self.arduino_port = port_names[0]
                self.arduino_handler = ArduinoHandler(self.arduino_port, self.tbprint)
